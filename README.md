...
ดึงออเดอร์ที่สร้างใน **Partner POS** เข้ามาวางแผนใน MRP แล้วเขียน **วันจัดส่งที่ทำได้จริง** กลับไปที่ Partner POS โดยอัตโนมัติ — ผ่าน Supabase โปรเจกต์เดียวกัน (แบบเดียวกับ khokai-oms)

**ต้องเชื่อม Supabase โปรเจกต์เดียวกับ Partner POS ก่อน** (แท็บ *ตั้งค่า & ข้อมูล*) — ใช้ anon key เดียวกัน แอปจะอ่าน/เขียนตาราง `khokai_partner_orders` และอ่าน `khokai_products` / `khokai_partners` ที่มีอยู่แล้ว (ไม่ต้องสร้างตารางเพิ่ม)

ขั้นตอนที่แท็บ **🔗 Partner POS**:
1. **🔄 โหลดออเดอร์จาก Partner POS** — ดึงออเดอร์ทั้งหมดมาแสดง (เลขที่ / ลูกค้า / วันส่ง / รายการ)
2. **จับคู่สินค้า** — สินค้าใน POS ↔ สินค้าใน MRP (ระบบจับคู่อัตโนมัติจากชื่อที่ตรงกัน แก้เองได้) เพราะ MRP เก็บค่ากำลังผลิต/BOM ของตัวเอง · เลือก "— ไม่นำเข้า —" เพื่อข้ามสินค้าที่ไม่ต้องการ
3. **⬇️ นำเข้าออเดอร์ที่เลือก** — สร้างออเดอร์ใน MRP (จำนวน = `pack_qty` = จำนวนถุงจริง, กำหนดส่ง = `delivery_date` ของ POS) แล้วไปดูผลที่แท็บ *ผลการวางแผน*
4. **☁️ เขียนวันส่งกลับ Partner POS** — เขียน "วันส่งได้จริง" (วันที่สินค้าทุกตัวในออเดอร์ผลิตเสร็จ = วันช้าสุด) กลับไปที่ช่อง `delivery_date` ของออเดอร์นั้นใน POS

> **ปลอดภัยต่อข้อมูล POS:** การเขียนกลับเป็น `update` เฉพาะช่อง `delivery_date` โดยจับคู่ด้วย `order_no` เท่านั้น — ไม่แตะรายการสินค้า / ยอดเงิน / สถานะการจ่าย (mirror หลัก data-ownership ของ khokai-oms)
>
> ⚠️ ถ้ากลับไป **แก้แล้วเซฟออเดอร์เดิมซ้ำใน Partner POS** ค่า `delivery_date` ที่เขียนกลับอาจถูกทับด้วยค่าในดราฟต์ของ POS — ให้เขียนวันส่งกลับ *หลัง* ปิดการแก้ใน POS แล้ว
